...
Usage:
    python aemo_full_chain.py [area]
...
async def main():
    ...
    parser.add_argument('area', nargs='?', default='NSW1', 
                        choices=['NSW1', 'VIC1', 'QLD1', 'SA1', 'TAS1'],
                        help='Area code (NSW1, VIC1, QLD1, SA1, TAS1)')
    ...
    try:
        ...
        print(f"Fetching AEMO data for area: {area}")
        raw_data = await api.fetch_raw_data(area=area)
        
        if not raw_data:
            print("Error: Failed to fetch data from AEMO API")
            return
