...
async def main():
    ...
    try:
        ...
        if not prices_5min:
            logger.debug("  Note: Raw 5-minute data not available in response (will use aggregated data)")
        ...
        if prices_5min:
            ...
            logger.info(f"  Expected 15-minute intervals: ~{expected_interval_count} ({source_5min_count} ÷ 3)")
            ...
            for ts_15min in sorted_timestamps[:5]:  # Check first 5 intervals
                ...
                if len(contributing_prices) == 3:
                    ...
                    if abs(aggregated_price - expected_avg) < 0.0001:
                        ...
                        logger.info(f"✅ {dt_15min.strftime('%H:%M')}: {contributing_prices} → avg={expected_avg:.4f} (matches {aggregated_price:.4f})")
                    else:
                        logger.warning(f"❌ {dt_15min.strftime('%H:%M')}: Expected {expected_avg:.4f}, got {aggregated_price:.4f}")
                else:
                    logger.debug(f"⚠️  {dt_15min.strftime('%H:%M')}: Only {len(contributing_prices)} contributing prices found")
        ...
        for i in range(len(sorted_timestamps) - 1):
            ...
            if diff_minutes == 15.0:
                ...
                logger.debug(f"  {dt1.strftime('%H:%M')} → {dt2.strftime('%H:%M')}: {diff_minutes:.0f} minutes")
        
        spacing_pct = (correct_spacing / total_checks * 100) if total_checks > 0 else 0
        logger.info(f"  Intervals with correct 15-minute spacing: {correct_spacing}/{total_checks} ({spacing_pct:.1f}%)")
        ...
        for time_key in sorted_keys[:8]:
            ...
            logger.info(f"  {dt.strftime('%Y-%m-%d %H:%M')} → {price_val:.4f} {target_currency}/kWh")
        ...
        if sorted_timestamps:
            ...
            logger.info(f"  Date range: {first_dt.strftime('%Y-%m-%d %H:%M')} to {last_dt.strftime('%Y-%m-%d %H:%M')}")
        ...
        if prices_5min:
            ...
            logger.info(f"✅ Aggregation: {len(prices_5min)} 5-min → {len(interval_raw_prices)} 15-min intervals (3:1 ratio)")
        else:
        ...
        if len(normalized_prices) >= TimeInterval.get_intervals_per_day():
            ...
            logger.info(f"✅ Nearly 1 full day of data ({len(normalized_prices)}/{TimeInterval.get_intervals_per_day()} intervals)")
            logger.info("\n" + "="*80)
            logger.info("TEST COMPLETED ✅")
            logger.info("="*80)
            return 0
        elif len(normalized_prices) > 0:
            logger.warning(f"⚠️  Partial day data ({len(normalized_prices)}/{TimeInterval.get_intervals_per_day()} intervals)")
            logger.info("\n" + "="*80)
