...
from custom_components.ge_spot.api.smard_adapter import SmardAdapter
from custom_components.ge_spot.const.sources import SOURCE_SMARD
...
async def main(market_area: str):
    """Main function to test SmardAdapter."""
    _LOGGER.info(f"Testing SmardAdapter for market area: {market_area}")

    async with aiohttp.ClientSession() as session:
        # Dummy ApiKeyManager, not used by SmardAdapter but required by BaseAPIAdapter constructor
        api_key_manager = ApiKeyManager(hass=None, config_entry_id="dummy_id")

        adapter = SmardAdapter(
            hass=None, 
            api_key_manager=api_key_manager,
            source_name=SOURCE_SMARD,
        ...
        try:
            ...
            if price_data.meta:
                _LOGGER.info(f"API URL Base: {price_data.meta.get('api_url_base')}")
                _LOGGER.info(f"Raw Unit: {price_data.meta.get('raw_unit')}")
                _LOGGER.info(f"Raw Index Preview: {price_data.meta.get('raw_index_preview')}")
                ...
                for i, price_entry in enumerate(price_data.hourly_raw):
                    if i < display_limit or i >= len(price_data.hourly_raw) - display_limit:
                        _LOGGER.info(
                            ...
                            f"Price: {price_entry['price']:.5f} {price_data.currency}/kWh" # SMARD provides in EUR/MWh, adapter converts to kWh
                        )
            ...
            _LOGGER.error(f"Error during SmardAdapter test: {e}", exc_info=True)

if __name__ == "__main__":
    market_area_arg = "DE-LU"
    supported_areas_info = "Supported areas: DE-LU, AT, BE, DK1, DK2, FR, NL, NO2, PL, CH, SI, CZ, HU, IT-NO"
