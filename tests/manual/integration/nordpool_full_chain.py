...
class DebugCacheManager(CacheManager):
    ...
    def __init__(self, hass, config):
        ...
        logger.debug(
            f"Debug Cache Manager initialized with config: {json.dumps(config, default=str)}"
        )
    ...
    def store(self, area, source, data, timestamp=None):
        ...
        cache_key = self._generate_cache_key(
            area, source, timestamp.date() if timestamp else datetime.now().date()
        )
        ...
        logger.debug(
            f"CACHE STATS after store: {len(cache_info.get('entries', {}))} entries in cache"
        )
        return result
    ...
    def get_data(self, area, target_date, source=None, max_age_minutes=None):
        ...
        if source:
            ...
            logger.debug(
                f"CACHE GET: Looking for any source for area '{area}' and date '{target_date}'"
            )
...
class DebugExchangeRateService(ExchangeRateService):
    ...
    async def _load_cache(self):
        ...
        try:
            ...
            if result:
                logger.debug(
                    f"EXCHANGE CACHE: Successfully loaded {len(self.rates)} currency rates"
                )
    ...
    async def _save_cache(self):
        ...
        logger.debug(
            f"EXCHANGE CACHE: Saving {len(self.rates)} currency rates to {self.cache_file}"
        )
...
async def main():
    ...
    elif area.startswith("NO") or area in [
        "Oslo",
        "Kr.sand",
        "Bergen",
        "Molde",
    ...
    tz_config = {"timezone_reference": "area"}  # Use area timezone as reference
        ...
        try:
            ...
            if not force_fetch:
                ...
                cached_data = cache_manager.get_data(
                    area=area, target_date=target_date, source=Source.NORDPOOL
                )
            ...
            if processed_data is None:
                ...
                raw_data_wrapper = await api.fetch_raw_data(
                ...
                if not interval_raw:  # Changed from hourly_raw
                    ...
                    logger.error(
                        "Error: No interval prices found after parsing the raw data"
                    )  # Updated message
                    return 1

                logger.info(
                    f"Found {len(interval_raw)} interval prices after parsing"
                )  # Changed from hourly
                ...
                if len(interval_raw) >= expected_intervals:
                    logger.info(
                        f"✓ Received sufficient data for at least one full day (expected: {expected_intervals})"
                    )
                else:
                    logger.warning(
                        f"⚠ Received fewer intervals than expected for one day (got: {len(interval_raw)}, expected: ≥{expected_intervals})"
                    )
                ...
                if len(sample_timestamps) >= 2:
                    ...
                    if abs(avg_interval - 15) < 1:  # Within 1 minute of 15
                        ...
                        logger.warning(
                            f"⚠ WARNING: Unexpected interval detected: {avg_interval:.1f} minutes"
                        )
                ...
                if len(normalized_prices) < 100:
                    logger.warning(
                        f"⚠️ Normalized prices ({len(normalized_prices)}) is less than expected (192)!"
                    )
                    logger.warning(
                        f"This suggests intervals are being aggregated or lost during normalization"
                    )
                ...
                logger.info(
                    f"Split into today ({len(today_prices)} intervals) and tomorrow ({len(tomorrow_prices)} intervals)"
                )
                ...
                if len(today_prices) + len(tomorrow_prices) < len(normalized_prices):
                    logger.warning(
                        f"⚠️ Lost {len(normalized_prices) - len(today_prices) - len(tomorrow_prices)} intervals during split!"
                    )
            ...
            all_original_prices = {}
            today_prices_display = processed_data.get(
                "interval_prices", {}
            )  # Changed from hourly_prices
            tomorrow_prices_display = processed_data.get(
                "tomorrow_interval_prices", {}
            )  # Changed from tomorrow_hourly_prices

            for hour, price_info in today_prices_display.items():
                all_original_prices[hour] = (
            ...
            logger.info(
                "\nInterval Prices (formatted as HH:MM in target timezone):"
            )  # Updated message
            logger.info(
                f"{'Time':<10} {f'{display_original_currency}/{display_source_unit}':<15} {f'{display_target_currency}/kWh':<15}"
            )
            logger.info("-" * 40)
            ...
            for hour_key in sorted(all_original_prices.keys()):
                ...
                converted_str = (
                    f"{converted_value:<15.6f}"
                    if isinstance(converted_value, (int, float))
                    else f"{str(converted_value):<15}"
                )
