    ...
    try:
        ...
        if not raw_data_response or "raw_data" not in raw_data_response:
            ...
            if not interval_prices and 'raw_data' in raw_data_response:
                logger.info("`interval_raw` not found or empty, attempting to parse from `raw_data` directly.")
                ...
                if nested_raw_data:
                    ...
                    if 'today' in nested_raw_data and nested_raw_data.get('today') and 'multiAreaEntries' in nested_raw_data['today']:
            ...
            if not interval_prices:
                logger.error("Could not find or extract any interval prices from the Nordpool response.")
                ...
                logger.info(f"Successfully obtained {len(interval_prices)} interval price points for processing.")
