...
async def test_single_area(api: EnergyChartsAPI, area: str):
    ...
    try:
        ...
        assert isinstance(raw_api_response, dict), f"raw_data should be dict, got {type(raw_api_response)}"
        ...
        assert len(unix_seconds) == len(prices), f"Array length mismatch: {len(unix_seconds)} vs {len(prices)}"
        
        # Validate metadata
        assert raw_data["timezone"] == "Europe/Berlin", f"Expected Europe/Berlin, got {raw_data['timezone']}"
        assert raw_data["currency"] == Currency.EUR, f"Expected EUR, got {raw_data['currency']}"
        assert raw_data["source"] == Source.ENERGY_CHARTS, f"Expected energy_charts, got {raw_data['source']}"
        ...
        assert parsed_data["source"] == Source.ENERGY_CHARTS, f"Expected energy_charts, got {parsed_data['source']}"
        assert parsed_data["currency"] == Currency.EUR, f"Expected EUR, got {parsed_data['currency']}"
        assert parsed_data["timezone"] == "Europe/Berlin", f"Expected Europe/Berlin, got {parsed_data['timezone']}"
        assert parsed_data["source_unit"] == "MWh", f"Expected MWh, got {parsed_data['source_unit']}"
        ...
        assert isinstance(interval_raw, dict), f"interval_raw should be dict, got {type(interval_raw)}"
        ...
        if len(sorted_timestamps) >= 2:
            ...
            for i in range(1, min(6, len(sorted_timestamps))):
                ...
                print(f"   {sorted_timestamps[i-1]} -> {sorted_timestamps[i]}: {diff_minutes:.0f} min")
                
                # Energy-Charts provides native 15-minute data
                assert 10 <= diff_minutes <= 20, f"Expected ~15 min intervals, got {diff_minutes} min"
