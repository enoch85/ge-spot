...
class TestBackwardConversions:
    ...
    def test_utc_to_multiple_timezones(self):
        ...
        for tz_name, (expected_hour, expected_min, expected_offset) in timezones.items():
            ...
            assert local_dt.hour == expected_hour, f"{tz_name}: Expected hour {expected_hour}, got {local_dt.hour}"
            assert local_dt.minute == expected_min, f"{tz_name}: Expected minute {expected_min}, got {local_dt.minute}"
            assert local_dt.strftime('%z') == expected_offset, f"{tz_name}: Expected offset {expected_offset}, got {local_dt.strftime('%z')}"
