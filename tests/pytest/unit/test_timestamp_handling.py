...
class TestTimestampHandling:
    ...
    def test_parse_timestamp_iso(self, parser, timezones):
        ...
        dt = parser.parse_timestamp(
            iso_tz, timezones["utc"]
        )  # Source TZ doesn't matter if offset present
        assert dt.hour == 12, f"Expected hour 12 UTC, got {dt.hour}"
        assert (
            dt.date() == datetime(2023, 5, 15).date()
        ), f"Expected date 2023-05-15, got {dt.date()}"
        ...
        dt = parser.parse_timestamp(
            iso_no_tz, timezones["cet"]
        )  # Assume CET (+1 or +2)
        # 14:00 CET is 13:00 UTC (standard) or 12:00 UTC (DST)
        assert dt.tzinfo == timezone.utc, f"Expected timezone UTC, got {dt.tzinfo}"
        # Check if the UTC hour matches the expected conversion from 14:00 CET
        expected_utc_hour = 14 - (
            timezones["cet"].utcoffset(datetime(2023, 5, 15, 14, 0)).total_seconds()
            / 3600
        )
        assert (
            dt.hour == expected_utc_hour
        ), f"Expected hour {expected_utc_hour} UTC from 14:00 CET, got {dt.hour}"
        assert (
            dt.date() == datetime(2023, 5, 15).date()
        ), f"Expected date 2023-05-15, got {dt.date()}"
        ...
        dt = parser.parse_timestamp(
            iso_offset, timezones["utc"]
        )  # Source TZ irrelevant
        # The time should be stored as UTC, so 18:45+02:00 becomes 16:45 UTC
        assert (
            dt.hour == 16
        ), f"Expected hour 16 UTC (after offset conversion), got {dt.hour}"  # FIXED Assertion
        ...
        ), f"Expected microsecond 123456, got {dt.microsecond}"
        assert dt.tzinfo == timezone.utc, f"Expected timezone UTC, got {dt.tzinfo}"
    ...
    def test_parse_timestamp_date_time(self, parser, timezones, test_dates):
        ...
        assert dt.tzinfo == timezone.utc, f"Expected timezone UTC, got {dt.tzinfo}"
        expected_utc_hour = 14 - (
            timezones["cet"].utcoffset(datetime(2023, 5, 15, 14, 30)).total_seconds()
            / 3600
        )
        assert (
            dt.hour == expected_utc_hour
        ), f"Expected hour {expected_utc_hour} UTC from 14:30 CET, got {dt.hour}"
        assert dt.minute == 30, f"Expected minute 30, got {dt.minute}"
        assert (
            dt.date() == datetime(2023, 5, 15).date()
        ), f"Expected date 2023-05-15, got {dt.date()}"
        ...
        dt = parser.parse_timestamp(
            us_date, timezones["us_eastern"]
        )  # Assume US Eastern source
        assert dt.tzinfo == timezone.utc, f"Expected timezone UTC, got {dt.tzinfo}"
        expected_utc_hour = 10 - (
            timezones["us_eastern"]
            .utcoffset(datetime(2023, 5, 15, 10, 45))
            .total_seconds()
            / 3600
        )
        assert (
            dt.hour == expected_utc_hour
        ), f"Expected hour {expected_utc_hour} UTC from 10:45 US Eastern, got {dt.hour}"
        assert dt.minute == 45, f"Expected minute 45, got {dt.minute}"
        assert (
            dt.date() == datetime(2023, 5, 15).date()
        ), f"Expected date 2023-05-15, got {dt.date}"
        ...
        assert dt.minute == 45, f"Expected minute 45, got {dt.minute}"
        assert (
            dt.date() == test_dates["today"]
        ), f"Expected today's date {test_dates['today']}, got {dt.date()}"
        assert dt.tzinfo == timezone.utc, f"Expected timezone UTC, got {dt.tzinfo}"
    ...
    def test_classify_timestamp_day(self, parser, timezones, test_dates):
        ...
        assert (
            day_type == "today"
        ), f"Expected 'today', got '{day_type}' for {today_utc} in UTC"
        ...
        assert (
            day_type == "tomorrow"
        ), f"Expected 'tomorrow', got '{day_type}' for {tomorrow_utc} in UTC"

        # Yesterday 12:00 UTC
        yesterday_utc = datetime.combine(
        ...
        day_type = parser.classify_timestamp_day(
            late_today_utc, timezones["utc"], test_dates["today"]
        )
        assert (
            day_type == "today"
        ), f"Expected 'today', got '{day_type}' for {late_today_utc} in UTC"

        # Should be "tomorrow" in Tokyo (UTC 22:00 Oct 3 = Tokyo 07:00 Oct 4, which is tomorrow from Oct 3)
        day_type = parser.classify_timestamp_day(
            late_today_utc, timezones["japanese"], test_dates["today"]
        )
        assert (
            day_type == "tomorrow"
        ), f"Expected 'tomorrow', got '{day_type}' for {late_today_utc} in Tokyo"

        # Test early tomorrow UTC (01:00) which might be today in US timezones (e.g. US Eastern UTC-5/-4 -> 21:00/20:00 previous day)
        early_tomorrow_utc = datetime.combine(
            test_dates["tomorrow"],
        ...
        assert (
            day_type == "tomorrow"
        ), f"Expected 'tomorrow', got '{day_type}' for {early_tomorrow_utc} in UTC"

        # Should be "today" in US Eastern (need to pass date_context to avoid defaulting to yesterday)
        day_type = parser.classify_timestamp_day(
            early_tomorrow_utc, timezones["us_eastern"], test_dates["today"]
        )
        assert (
            day_type == "today"
        ), f"Expected 'today', got '{day_type}' for {early_tomorrow_utc} in US Eastern"  # FIXED Assertion
    ...
    def test_normalize_timestamps(self, parser, timezones, test_dates):
        ...
        assert (
            len(result["today"]) == 3
        ), f"Expected 3 today entries, got {len(result['today'])}: {result['today']}"  # FIXED Assertion
        assert (
            len(result["tomorrow"]) == 2
        ), f"Expected 2 tomorrow entries, got {len(result['tomorrow'])}: {result['tomorrow']}"
        assert (
            len(result["other"]) == 0
        ), f"Expected 0 other entries, got {len(result['other'])}"

        # Verify specific hours are correctly assigned
        assert (
            result["today"]["10:00"] == 50.0
        ), "Expected '10:00' today to have value 50.0"
        assert (
            result["today"]["12:00"] == 60.0
        ), "Expected '12:00' today to have value 60.0"
        assert (
            result["today"]["14:00"] == 70.0
        ), "Expected '14:00' today to have value 70.0"  # FIXED Assertion

        assert (
            result["tomorrow"]["10:00"] == 55.0
        ), "Expected '10:00' tomorrow to have value 55.0"
        assert (
            result["tomorrow"]["12:00"] == 65.0
        ), "Expected '12:00' tomorrow to have value 65.0"
        ...
        result_cet = parser.normalize_timestamps(
            ...
            date_context=test_dates[
                "today"
            ],  # Provide date context to avoid timezone-based date drift
        )
        ...
        assert (
            expected_hour_10_cet in result_cet[day_key_10]
        ), f"Expected '{expected_hour_10_cet}' in '{day_key_10}' CET data"
        if expected_hour_10_cet in result_cet[day_key_10]:
            assert (
                result_cet[day_key_10][expected_hour_10_cet] == 50.0
        ...
        dt_14_utc = parser.parse_timestamp(
            "14:00", timezones["utc"]
        )  # Uses today's date
        ...
        day_key_14 = (
            "today" if expected_hour_14_aus in result_aus["today"] else "tomorrow"
        )

        assert (
            expected_hour_14_aus in result_aus[day_key_14]
        ...
        if expected_hour_14_aus in result_aus[day_key_14]:
            assert (
                result_aus[day_key_14][expected_hour_14_aus] == 70.0
    ...
    def test_real_world_edge_cases(self, parser, timezones, test_dates):
        ...
        assert (
            dst_fallback_date is not None
        ), "Could not determine DST fallback date for testing"
        ...
        assert (
            result_dst is not None
        ), "Normalizing DST transition timestamps should not fail"
        # All these times should fall on the same calendar day in CET
        assert "today" in result_dst, "Result should contain 'today' key"
        assert (
            len(result_dst["today"]) == 2
        ), f"Expected 2 distinct hours (02:00, 04:00) after DST fallback normalization, got {len(result_dst['today'])} keys: {result_dst['today'].keys()}"  # FIXED Assertion: 2 keys expected (02:00, 04:00)
        assert (
            "02:00" in result_dst["today"]
        ), "Expected '02:00' key after DST fallback normalization"
        # Check that the value corresponds to the *second* occurrence (standard time, +01:00) due to overwrite
        assert (
            result_dst["today"]["02:00"] == 85.0
        ), "Expected value from the second occurrence of 02:00 during DST fallback"
        assert "04:00" in result_dst["today"], "Expected '04:00' key"
        assert result_dst["today"]["04:00"] == 90.0, "Expected correct value for 04:00"
        assert (
            len(result_dst["tomorrow"]) == 0
        ), "Expected 0 tomorrow entries for DST test"
        assert (
            len(result_dst["other"]) == 0
        ), f"Expected 0 other entries for DST test, got {len(result_dst['other'])}"  # Check 'other'
        ...
        result_empty = parser.normalize_timestamps(
            empty_prices,
            timezones["utc"],
            timezones["utc"],  # Source timezone  # Target timezone
        )

        # Should return empty structures, not None or error
        assert result_empty is not None, "Normalizing empty prices should not fail"
        assert (
            "today" in result_empty
        ), "Result should contain 'today' key even with empty input"
        assert (
            "tomorrow" in result_empty
        ), "Result should contain 'tomorrow' key even with empty input"
        assert (
            "other" in result_empty
        ), "Result should contain 'other' key even with empty input"
        assert (
            len(result_empty["today"]) == 0
        ), "Expected 0 entries for today with empty input"
        assert (
            len(result_empty["tomorrow"]) == 0
        ), "Expected 0 entries for tomorrow with empty input"
        assert (
            len(result_empty["other"]) == 0
        ), "Expected 0 entries for other with empty input"
        ...
        ), "Normalizing malformed timestamps should not return None"
        # Assert that only the valid entry was created
        assert (
            len(result_malformed["today"]) == 1
        ), f"Expected 1 today entry with mixed malformed/valid input, got {len(result_malformed['today'])}"
        assert (
            len(result_malformed["tomorrow"]) == 0
        ), f"Expected 0 tomorrow entries with mixed malformed/valid input, got {len(result_malformed['tomorrow'])}"
        assert (
            len(result_malformed["other"]) == 0
        ), f"Expected 0 other entries with mixed malformed/valid input, got {len(result_malformed['other'])}"
        assert (
            "15:00" in result_malformed["today"]
        ), "Expected valid timestamp '15:00' to be present"
        assert (
            result_malformed["today"]["15:00"] == 99.0
        ), "Expected correct value for valid timestamp"
