...
SAMPLE_NORDPOOL_RESPONSES = {
    "SE3": {
        ...
        "multiAreaEntries": lambda: generate_15min_intervals(
            get_test_date(), BASE_PRICES_SE3, "SE3"
        ),
        "currency": "EUR",
        "exchangeRate": 1,
    ...
    "NO1": {
        ...
        "market": "DayAhead",
        "multiAreaEntries": lambda: generate_15min_intervals(
            get_test_date(), BASE_PRICES_NO1, "NO1"
        ),
        "currency": "EUR",
        "exchangeRate": 1,
    ...
    "DK1": {
        ...
        "market": "DayAhead",
        "multiAreaEntries": lambda: generate_15min_intervals(
            get_test_date(), BASE_PRICES_DK1, "DK1"
        ),
        "currency": "EUR",
        "exchangeRate": 1,
...
SAMPLE_NORDPOOL_TOMORROW_RESPONSES = {
    "SE3": {
        ...
        "market": "DayAhead",
        "multiAreaEntries": lambda: generate_15min_intervals(
            get_tomorrow_date(), BASE_PRICES_TOMORROW, "SE3"
        ),
        "currency": "EUR",
        "exchangeRate": 1,
        "areaStates": [{"state": "Final", "areas": ["SE3"]}],
...
MOCK_EXCHANGE_RATES = {
    "rates": {"SEK": 11.0, "NOK": 10.5, "DKK": 7.45, "EUR": 1.0, "USD": 1.1},
    "base": "EUR",
}
...
async def test_nordpool_live_fetch_parse(area, monkeypatch):
    ...
    try:
        ...
        assert (
            raw_data.get("timezone") == "Europe/Oslo"
        ), f"Timezone should be Europe/Oslo, got {raw_data.get('timezone')}"
        ...
        assert (
            len(multi_area_entries) > 0
        ), f"No multiAreaEntries found for {area} - this indicates a real issue with the API"
        ...
        if multi_area_entries:
            ...
            assert isinstance(
                first_entry, dict
            ), f"Entry should be a dictionary, got {type(first_entry)}"
            assert (
        ...
        assert (
            parsed_data.get("source") == Source.NORDPOOL
        ), f"Source should be {Source.NORDPOOL}, got {parsed_data.get('source')}"
        assert (
            parsed_data.get("area") == area
        ...
        ), f"Currency should be {Currency.EUR}, got {parsed_data.get('currency')}"

        # Timezone validation
        assert (
            parsed_data.get("timezone") == "Europe/Oslo"
        ), f"Timezone should be Europe/Oslo, got {parsed_data.get('timezone')}"
        ...
        assert (
            min_expected <= len(interval_prices) <= max_expected
        ), f"Expected {min_expected}-{max_expected} interval prices (15-minute data), got {len(interval_prices)}"
        ...
        for timestamp, price in interval_prices.items():
            ...
            assert (
                -500 <= price <= 3000
            ), f"Price {price} EUR/MWh for {timestamp} is outside reasonable range for {area}"

        # Check for sequential 15-minute intervals
        timestamps = sorted(interval_prices.keys())
        interval_diffs = []
        for i in range(
            1, min(97, len(timestamps))
        ):  # Check first 96 intervals (1 day of 15-min data)
        ...
        logger.info(
            f"Nordpool Test ({area}): PASS - Found {len(interval_prices)} interval prices. "
