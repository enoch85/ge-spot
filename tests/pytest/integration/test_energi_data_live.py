...
pytestmark = [
    pytest.mark.liveapi,
    pytest.mark.skip(reason="Live API test - requires network access, skip by default"),
]
...
async def test_energi_data_live_fetch_parse(area):
    ...
    try:
        ...
        ), f"Currency should be {Currency.DKK}, got {parsed_data.get('currency')}"

        # Timezone validation
        api_timezone = parsed_data.get("timezone")
        assert (
            api_timezone is not None and api_timezone
        ), f"timezone should have a value, got {api_timezone}"
        # Danish timezone should be Europe/Copenhagen
        assert (
            "Europe/Copenhagen" in api_timezone or "CET" in api_timezone
        ), f"Expected Copenhagen timezone, got {api_timezone}"
        ...
        ), f"Expected at least {min_expected_intervals} interval prices (15-min native), got {len(interval_prices)}"
        ...
        for timestamp, price in interval_prices.items():
            ...
            try:
                ...
                seven_days_ago = now - timedelta(
                    days=7
                )  # Energi Data Service can provide historical data
                three_days_ahead = now + timedelta(
                    days=3
                )  # And some future data (day-ahead market)
                assert (
            ...
            assert isinstance(
                price, (int, float)
            ), f"Price should be numeric, got {type(price)} for timestamp {timestamp}"
            ...
            assert (
                -500 <= price <= 5000
            ), f"Price {price} DKK/MWh for {timestamp} is outside reasonable range for {area}"
        ...
        for i in range(1, min(len(timestamps), 20)):  # Check first 20 intervals
            ...
            if time_diff_minutes == 15:
                ...
                logger.warning(
                    f"Unexpected time gap of {time_diff_minutes} minutes between {timestamps[i-1]} and {timestamps[i]}"
                )
        ...
        assert (
            spacing_pct >= 80
        ), f"Expected mostly 15-minute intervals, got {spacing_pct:.1f}% correct spacing"

        logger.info(
            f"Energi Data Service Live Test ({area}): PASS - Found {len(interval_prices)} prices. "
