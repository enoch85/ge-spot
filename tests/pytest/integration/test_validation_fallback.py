...
<Publication_MarketDocument>
    <TimeSeries>
        ...
        assert "error" not in result or not result["error"], f"Should not have error, got: {result.get('error')}"
        ...
        assert len(result.get("attempted_sources", [])) >= 1, "Should have attempted at least one source"
    ...
    async def test_data_processor_validation_rejects_future_only_data(self):
        ...
        mock_manager.is_in_grace_period.return_value = False  # NOT in grace period - strict validation
...
<Publication_MarketDocument>
    <TimeSeries>
        ...
        with patch.object(processor, '_ensure_exchange_service', AsyncMock()):
            ...
            assert "validation" in result["error"].lower() or "current interval" in result["error"].lower(), \
