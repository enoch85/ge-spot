...
class GSpotOptionsFlow(config_entries.OptionsFlow):
    ...
    async def async_step_init(self, user_input=None):
        ...
        schema = {
            vol.Optional(
                CONF_VAT, 
                default=options.get(CONF_VAT, self.config_entry.data.get(CONF_VAT, DEFAULT_VAT))
            ): vol.All(vol.Coerce(float), vol.Range(min=0, max=1)),
            vol.Optional(
                CONF_UPDATE_INTERVAL, 
                default=options.get(CONF_UPDATE_INTERVAL, self.config_entry.data.get(CONF_UPDATE_INTERVAL, DEFAULT_UPDATE_INTERVAL))
        ...
        if source == SOURCE_ENTSO_E:
            schema[vol.Optional(
                "api_key", 
                default=options.get("api_key", self.config_entry.data.get("api_key", ""))
            )] = cv.string
