...
class DataProcessor:
    ...
    def _calculate_processing_config_hash(self) -> str:
        """Calculate hash of configuration that affects data processing.
        
        This hash is used to detect when cached processed data is stale.
        If ANY of these parameters change, the hash will differ, triggering reprocessing:
        - target_currency (EUR, SEK, etc.)
        - vat_rate (0.25 for 25%)
        - include_vat (True/False)
        - display_unit (kWh, MWh, cents)
        - precision (decimal places)
        - target_timezone (Europe/Stockholm, etc.) - affects interval keys
        
        Returns:
            12-character MD5 hash string of all processing configuration parameters
        """
        import hashlib
        config_str = (
            f"{self.target_currency}_"
            f"{self.vat_rate}_"
            f"{self.include_vat}_"
            f"{self.display_unit}_"
            f"{self.precision}_"
            f"{str(self._tz_service.target_timezone)}"
        )
        return hashlib.md5(config_str.encode()).hexdigest()[:12]


    def _is_processed_data_valid(self, data: Dict[str, Any]) -> bool:
        """Check if cached processed data is valid for current configuration.
        
        Processed data is ONLY valid if ALL of these conditions are met:
        1. It has processed format (HH:MM keys, statistics, etc.)
        2. Processing config hasn't changed (VAT, currency, display unit, precision, timezone)
        3. Raw data is still available for reprocessing if needed
        4. Data is for the current date (not stale from yesterday)
        5. Data is sufficiently complete (at least 80% of today's intervals)
        
        If ANY check fails, we MUST reprocess from raw data!
        
        Args:
            data: Cached data dictionary
            
        Returns:
            True if processed data is valid and can be used as-is
        """
        # First check: Does it have processed data?
        has_interval_prices = bool(data.get("interval_prices"))
        has_statistics = bool(data.get("statistics"))
        has_target_tz = data.get("target_timezone") is not None
        
        if not (has_interval_prices and has_statistics and has_target_tz):
            _LOGGER.debug(f"[{self.area}] Cache missing processed data markers")
            return False
        
        # Second check: Is interval_prices in processed format (HH:MM)?
        interval_prices = data.get("interval_prices", {})
        if not interval_prices:
            return False
            
        first_key = next(iter(interval_prices.keys()), None)
        if not first_key:
            return False
        
        # Processed keys are "HH:MM", raw keys are ISO timestamps
        is_processed_format = ":" in first_key and len(first_key) <= 5
        if not is_processed_format:
            _LOGGER.debug(f"[{self.area}] Cache has raw format, not processed")
            return False
        
        # CRITICAL CHECK: Has processing config changed?
        cached_config_hash = data.get("processing_config_hash")
        current_config_hash = self._calculate_processing_config_hash()
        
        if cached_config_hash != current_config_hash:
            _LOGGER.warning(
                f"[{self.area}] Processing config changed! "
                f"(cached hash: {cached_config_hash}, current: {current_config_hash}). "
                f"Will reprocess from raw data."
            )
            return False
        
        # CRITICAL CHECK: Do we have raw data for reprocessing?
        # Without raw_interval_prices_original, we can't reprocess if needed
        has_raw_data = bool(data.get("raw_interval_prices_original"))
        has_source_tz = bool(data.get("source_timezone"))
        has_source_currency = bool(data.get("source_currency"))
        
        if not (has_raw_data and has_source_tz and has_source_currency):
            _LOGGER.warning(
                f"[{self.area}] Cached processed data missing raw fields needed for reprocessing. "
                f"raw_data={has_raw_data}, source_tz={has_source_tz}, source_currency={has_source_currency}"
            )
            return False
        
        # CRITICAL CHECK: Is the data still current for today?
        # Check if the interval_prices contain intervals for the current date
        from homeassistant.util import dt as dt_util
        now = dt_util.now()
        current_date = now.date()
        
        # Get expected intervals for today from timezone service
        expected_today_keys = set(self._tz_service.get_today_range())
        actual_keys = set(interval_prices.keys())
        
        # Check if we have ANY of today's intervals in the cached data
        # If the cache is from yesterday, today's intervals won't be present
        matching_keys = expected_today_keys & actual_keys
        
        if not matching_keys:
            _LOGGER.warning(
                f"[{self.area}] Cached processed data doesn't contain intervals for today "
                f"({current_date}). Cache is stale and must be reprocessed."
            )
            return False
        
        # Check data completeness: We should have most of today's intervals (at least 80%)
        from ..const.time import TimeInterval
        expected_intervals = TimeInterval.get_intervals_per_day()
        min_required = int(expected_intervals * 0.8)
        
        if len(matching_keys) < min_required:
            _LOGGER.warning(
                f"[{self.area}] Cached processed data incomplete for today: "
                f"{len(matching_keys)}/{expected_intervals} intervals (need {min_required}). "
                f"Will reprocess from raw data."
            )
            return False
        
        # All checks passed - processed data is valid and current
        _LOGGER.debug(
            f"[{self.area}] Cached processed data is valid: "
            f"{len(matching_keys)}/{expected_intervals} today's intervals, "
            f"config hash matches, raw data available"
        )
        return True


    async def _update_current_next_only(self, processed_data: Dict[str, Any]) -> Dict[str, Any]:
        """Update only current_price and next_interval_price from cached processed data.
        
        This is the fast path for cached data that's already been normalized,
        converted, and has statistics calculated. We only need to update which
        interval is "current" since time has advanced.
        
        IMPORTANT: This method is ONLY called when processing config is unchanged!
        If VAT/currency/display_unit changed, we reprocess from raw data instead.
        
        Args:
            processed_data: Already processed cached data (validated via _is_processed_data_valid)
            
        Returns:
            Updated processed data with current/next prices refreshed
        """
        from homeassistant.util import dt as dt_util
        
        # Get current interval keys (these might have changed since caching)
        current_interval_key = self._tz_service.get_current_interval_key()
        next_interval_key = self._tz_service.get_next_interval_key()
        
        # Update the interval key fields
        processed_data["current_interval_key"] = current_interval_key
        processed_data["next_interval_key"] = next_interval_key
        
        # Look up prices in already-processed data
        interval_prices = processed_data.get("interval_prices", {})
        tomorrow_prices = processed_data.get("tomorrow_interval_prices", {})
        
        # Update current price (check today first, then tomorrow for late evening)
        processed_data["current_price"] = (
            interval_prices.get(current_interval_key) or 
            tomorrow_prices.get(current_interval_key)
        )
        
        # Update next price
        processed_data["next_interval_price"] = (
            interval_prices.get(next_interval_key) or 
            tomorrow_prices.get(next_interval_key)
        )
        
        # Update last_update timestamp to now
        processed_data["last_update"] = dt_util.now().isoformat()
        
        # Ensure using_cached_data flag is set
        processed_data["using_cached_data"] = True
        
        _LOGGER.debug(
            f"[{self.area}] Fast-path cache update: current={processed_data['current_price']}, "
            f"next={processed_data['next_interval_price']}"
        )
    ...
    async def process(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # NEW: Check if data is already fully processed AND config hasn't changed
        if is_cached_data:
            # Check if processed data is valid for current config
            if self._is_processed_data_valid(data):
                _LOGGER.debug(f"[{self.area}] Cached processed data valid, updating current/next prices only")
                return await self._update_current_next_only(data)
            else:
                _LOGGER.info(f"[{self.area}] Cached processed data stale (config changed), reprocessing from raw data")
                # Fall through to reprocess from raw_interval_prices_original
        ...
        processed_result = {
            ...
            # Add config hash for staleness detection
            "processing_config_hash": self._calculate_processing_config_hash(),
        }
