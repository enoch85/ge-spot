        ...
        if Source.ENTSOE in self.config.get(Config.SOURCE_PRIORITY, []):
            ...
            if api_key:
                try:
                    ...
                    _LOGGER.debug(f"ENTSO-E API key status for region {area}: {'valid' if is_valid else 'invalid'}")
                    ...
                    if not is_valid and self.config.get(Config.SOURCE_PRIORITY):
                        ...
                        _LOGGER.warning(f"Detected invalid ENTSOE API key, adjusted source priority: {adjusted_priority}")
