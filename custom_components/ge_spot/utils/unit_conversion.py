...
def convert_export_price(
    spot_price: float,
    source_unit: str,
    target_unit: str = EnergyUnit.TARGET,  # Default target is kWh
    multiplier: float = 1.0,  # Multiplier applied to spot price
    offset: float = 0.0,  # Offset added after multiplier (can be negative)
    vat_rate: float = 0.0,  # Export VAT rate (often 0%)
    display_unit_multiplier: int = 1,  # Multiplier for subunits (e.g. 100 for cents)
    offset_in_subunit: bool = False,  # Whether offset is entered in subunit (cents/øre)
) -> Optional[float]:
    """Calculate export/production price from spot price.

    Export prices use formula: (spot_price × multiplier + offset) × (1 + VAT)

    This is common for prosumers who sell electricity back to the grid at rates
    that differ from import prices. Many providers use a fraction of the spot
    price plus/minus a fixed offset.

    Example (Belgium):
    - Import: (0.1068 × BELPEX + 1.500) × 1.06 EUR cents/kWh
    - Export: (0.1 × BELPEX - 1.150) EUR cents/kWh (VAT exempt)

    Args:
        spot_price: The raw spot price value (in source currency/unit).
        source_unit: The energy unit of the spot price (e.g. EnergyUnit.MWH).
        target_unit: The target energy unit (e.g. EnergyUnit.KWH).
        multiplier: Multiplier applied to spot price (e.g. 0.1 for 10% of spot).
        offset: Offset added after multiplier, in target unit (can be negative).
        vat_rate: VAT rate for export (0 to 1). Often 0% for feed-in tariffs.
        display_unit_multiplier: Multiplier for display subunits (e.g. 100 for cents).
        offset_in_subunit: If True, offset is in subunit (cents/øre), else main unit.

    Returns:
        The calculated export price, or None if calculation is not possible.
    """
    if spot_price is None:
        return None

    try:
        # 1. Convert energy units first (spot price from MWh to kWh typically)
        converted_price = spot_price
        if source_unit != target_unit:
            source_factor = EnergyUnit.CONVERSION.get(source_unit)
            target_factor = EnergyUnit.CONVERSION.get(target_unit)
            ...
            if source_unit == EnergyUnit.MWH and target_unit == EnergyUnit.KWH:
                converted_price = spot_price / 1000
            elif source_unit == EnergyUnit.KWH and target_unit == EnergyUnit.MWH:
                converted_price = spot_price * 1000
            else:
                converted_price = spot_price * (source_factor / target_factor)

        # 2. Apply multiplier to the converted spot price
        price = converted_price * multiplier

        # 3. Add offset (can be negative for feed-in scenarios)
        # If offset is entered in subunit, convert to main unit first
        offset_to_add = offset
        if offset_in_subunit and display_unit_multiplier > 1:
            offset_to_add = offset / display_unit_multiplier
        price += offset_to_add

        # 4. Apply export VAT (often 0% for feed-in tariffs)
        price *= 1 + vat_rate
