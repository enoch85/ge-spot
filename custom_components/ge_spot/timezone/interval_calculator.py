...
class IntervalCalculator:
    ...
    def get_current_interval_key(self) -> str:
        ...
        _LOGGER.debug(f"IntervalCalculator input: timezone={self.timezone}, system_timezone={self.system_timezone}, area_timezone={self.area_timezone}, timezone_reference={self.timezone_reference}")
        ...
        if self.timezone_reference == TimezoneReference.HOME_ASSISTANT:
            if self.area_timezone and self.area_timezone != self.system_timezone:
                ...
                _LOGGER.debug(f"IntervalCalculator compensation: now_system={now_system}, now_area={now_area}, hour_diff={hour_diff}")
                ...
                _LOGGER.debug(f"Applied timezone compensation of {-hour_diff} hours: {rounded.hour}:{rounded.minute:02d} → {interval_key}")
                return interval_key
        ...
        # If area timezone is provided and using Local Area Time mode, use it for determining the current interval
            ...
            _LOGGER.debug(f"IntervalCalculator using area_timezone: now_display={now_display}, area_timezone={self.area_timezone}")
        else:
            # Otherwise use system timezone for display
            now_display = now.astimezone(self.system_timezone)
            _LOGGER.debug(f"IntervalCalculator using system_timezone: now_display={now_display}, system_timezone={self.system_timezone}")
        ...
        _LOGGER.debug(f"IntervalCalculator DST: is_transition={is_transition}, transition_type={transition_type}, now={now}")
        ...
        if is_transition and transition_type == DSTTransitionType.FALL_BACK and now.hour == 2:
            ...
            if dst_offset > 0:
                ...
                _LOGGER.debug("Current interval during fall back DST transition: second 02:XX (03:XX)")
        ...
        _LOGGER.debug(f"IntervalCalculator result: interval_key={interval_key}, rounded={rounded}, display_tz={display_tz}")
        return interval_key
        ...
        if is_transition:
            if transition_type == DSTTransitionType.SPRING_FORWARD and now_display.hour == 1:
                ...
                if next_interval.hour == 2:
                    # Skip to 3:00
                    _LOGGER.debug("Next interval during spring forward: skipping from 01:XX to 03:00")
                    return "03:00"
                ...
                if dst_offset > 0:
                    ...
                    _LOGGER.debug("Next interval during fall back first 02:XX: moving forward normally")
                    return f"{next_interval.hour:02d}:{next_interval.minute:02d}"
                else:
                    # Second time through 2:XX, next is normal 3:XX
                    _LOGGER.debug("Next interval during fall back second 02:XX: normal next interval")
                    return f"{next_interval.hour:02d}:{next_interval.minute:02d}"

        # Log next interval determination for debugging
        next_interval_key = f"{next_interval.hour:02d}:{next_interval.minute:02d}"
        _LOGGER.debug(f"IntervalCalculator result: next_interval_key={next_interval_key}, next_interval={next_interval}")
    ...
    def get_interval_key_for_datetime(self, dt: datetime) -> str:
        ...
        _LOGGER.debug(f"IntervalCalculator input: dt={dt}, timezone_reference={self.timezone_reference}, area_timezone={self.area_timezone}, system_timezone={self.system_timezone}")
        ...
        if self.timezone_reference == TimezoneReference.LOCAL_AREA and self.area_timezone:
            ...
            _LOGGER.debug(f"IntervalCalculator using area_timezone: target_dt={target_dt}, area_timezone={self.area_timezone}")
        # Otherwise use Home Assistant Time mode
        else:
            target_dt = dt.astimezone(self.system_timezone)
            _LOGGER.debug(f"IntervalCalculator using system_timezone: target_dt={target_dt}, system_timezone={self.system_timezone}")
